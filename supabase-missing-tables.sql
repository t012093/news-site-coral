-- Supabase不足テーブル作成用SQL
-- usersテーブルが既に存在する場合、このスクリプトで残りのテーブルを作成

-- Enable necessary extensions (再実行しても安全)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Projects table (usersが存在する前提)
...
-- Create indexes for better performance (安全に実行)
...
-- Create updated_at trigger function (安全に実行)
...
-- Apply updated_at trigger to all tables (DROP IF EXISTS で安全に)
...
-- Row Level Security (RLS) policies (安全に実行)
...
-- Basic RLS policies (DROP IF EXISTS で安全に)
